...
class TestRunner {
  ...
  mount(container) {
    ...
    this.tests.forEach((test, index) => {
      ...
      row.querySelector("button").addEventListener("click", () => this.runOne(index));
  ...
  async runAll() {
    ...
    document.getElementById("run-state").className = `badge ${failed ? "fail" : "pass"}`;
  }
}
...
runner.add("createSignal + effect", "更新同值不触发，函数式 setter 可用。", async () => {
...
runner.add("dependency cleanup", "条件分支切换后不会继续订阅旧 signal。", async () => {
...
runner.add("createMemo", "依赖变更时重算；memo 结果相同不唤醒下游 effect。", async () => {
...
runner.add("untrack + watch", "untrack 不建立订阅；watch defer 首次不执行。", async () => {
...
runner.add("createStore shallow", "浅层 store 只追踪直接属性。", async () => {
  ...
  await tick();
  equal(profileRuns, before, "nested mutation should not trigger shallow store");
});
...
runner.add(
  ...
  async () => {
    ...
    state.rows.push({ id: "green-l", color: "Green", size: "L", stock: 1, price: 109 });
    await tick();
...
runner.add(
  ...
  async () => {
    ...
    const total = createMemo(() => state.skus.reduce((sum, sku) => sum + sku.stock, 0));
    const low = createMemo(() => state.skus.filter((sku) => sku.stock < 5).length);
    const value = createMemo(() => state.skus.reduce((sum, sku) => sum + sku.stock * sku.price, 0));
...
runner.add("bindText + insert + Show", "DOM 文本和条件节点细粒度更新。", async () => {
...
runner.add("bindList keyed reconciliation", "keyed list 复用节点并根据排序移动 DOM。", async () => {
  ...
  await tick();
  equal(textOf(container), "CBAD", "DOM order should follow keyed array order");
  assert(nodes.get("b") === bNode, "existing keyed node should be reused");
});

runner.add("JSX runtime factory", "h/jsx 运行时支持动态 props、children 和事件。", async () => {
  const [count, setCount] = createSignal(0);
  ...
  view.click();
  await tick();
  equal(view.textContent, "count 2", "event should update signal-backed child");
...
runner.add("createResource latest-wins", "source 快速变化时只保留最新请求结果。", async () => {
...
runner.add("createRoot disposal", "dispose 后 effect 不再响应后续更新。", async () => {
  const [count, setCount] = createSignal(0);
  let runs = 0;
...
runner.add("startTransition", "低优先级队列异步刷新，最终状态一致。", async () => {
...
function createSkuState() {
  return createDeepStore({
    next: 4,
    rows: [
      { id: "sku-1", name: "Tee", color: "Black", size: "S", stock: 12, price: 89 },
      { id: "sku-2", name: "Tee", color: "Black", size: "M", stock: 4, price: 89 },
      { id: "sku-3", name: "Hoodie", color: "Green", size: "L", stock: 7, price: 199 },
    ],
  });
}
...
function mountSkuDemo() {
  ...
  skuDispose = createRoot((dispose) => {
    ...
    const lowStock = createMemo(() => skuState.rows.filter((row) => Number(row.stock) < 5).length);
    ...
    bindText(summary, () => `总库存 ${totalStock()} / 低库存 ${lowStock()} / 货值 ${totalValue()}`);
      ...
      (row) => {
        const tr = jsx`
          <tr>
            ...
            <td>
              <input type="number" min="0" value=${() => row.stock} onInput=${(event) => {
                row.stock = Number(event.currentTarget.value);
              }}>
            </td>
            <td>
              <input type="number" min="0" value=${() => row.price} onInput=${(event) => {
...
document.getElementById("run-all").addEventListener("click", () => runner.runAll());
